...
@Component({
  selector: 'app-dialogo-tarifariodelmateriales',
  templateUrl: './dialogo-tarifariodelmateriales.component.html',
  styleUrls: ['./dialogo-tarifariodelmateriales.component.css']
})
export class DialogoTarifarioDeMaterialesComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogo: MatDialogRef<DialogoTarifarioDeMaterialesComponent>,
  ...
  buscarServicio(IdTarifario: number): void {
    ...
    this.solicitudesService.getListarMaterialesDelTarifario(IdTarifario).then((respuesta) => {
  ...
  listarTarifado() {
    this.solicitudesService.getListarTarifarioMaterial(this.IdSolicitud).then((respuesta) => {
      if (respuesta.TipoResultado == 1) {
