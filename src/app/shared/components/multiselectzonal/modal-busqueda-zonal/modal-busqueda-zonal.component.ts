...
export interface ZonalElement {
  Id: number;
  Nombre: string;
}
import {MatTableDataSource} from '@angular/material/table';
import {ZonalService} from "@shared/services/zonal.service";
const ELEMENT_DATA: ZonalElement[] = [
  // {Nombre: 'segundo mike', NombreCorto: 'segundo', NumeroDocumento: '70116577'},
];
@Component({
  selector: 'app-modal-busqueda-zonal',
  templateUrl: './modal-busqueda-zonal.component.html',
  styleUrls: ['./modal-busqueda-zonal.component.css']
})
export class BusquedaZonalComponent implements OnInit {
  IdsClientes:string="";
  isLoading = false;
  displayedColumns: string[] = ['select', 'Nombre'];
  listDocumentoSeleccionado: any = [];
  @ViewChild('paginator')paginator!: MatPaginator;
  dataSource = new MatTableDataSource<ZonalElement>(ELEMENT_DATA);
  selection = new SelectionModel<ZonalElement>(true, []);
  constructor(
    private clienteService: ZonalService,
    public dialogo: MatDialogRef<BusquedaZonalComponent>,
  ...
  eventEmitDoubleClick(event:any){   
    ...
    let ELEMENT_DATA_SELECCIONADO: ZonalElement[] = this.dataSource.data.filter(x => {
  ...
  buscarCliente(nombre:string): void {
    this.isLoading = true;
    this.clienteService.getZonalFiltro({
