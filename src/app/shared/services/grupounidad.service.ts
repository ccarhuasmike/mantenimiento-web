...
export class GrupoUnidadService {
  ...
  async getGrupoUnidadFiltro(req: any): Promise<any> {
    var idcliente = await this.encriptar(req.idcliente);
    var nombre = await this.encriptar(req.nombre);
    return new Promise((resolve, reject) => {
      this.http
        .get(`${this.apiEdiUrl}/ADM/UnidadMantenimiento/BuscarGrupoUnidadMantenimientoActivosFiltro?pIdCliente=${idcliente.data}&pNombre=${nombre.data}`)
        .subscribe({
          next: (data:any) => {            
