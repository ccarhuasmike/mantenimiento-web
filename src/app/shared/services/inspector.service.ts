...
export class InspectorService {
  ...
  async getInspectorFiltro(req: any): Promise<any> {
    var idcliente = await this.encriptar(req.idcliente);
    var nombre = await this.encriptar(req.nombre);
    return new Promise((resolve, reject) => {
      this.http
        .get(`${this.apiEdiUrl}/INS/ListarInspectorPorIdsClientes?pIdCliente=${idcliente.data}&pNombre=${nombre.data}`)
        .subscribe({
          next: (data:any) => {
